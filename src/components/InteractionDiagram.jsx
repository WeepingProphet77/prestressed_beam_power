  ...
  return (
    <div className="beam-diagram interaction-diagram">
      ...
      <svg viewBox={`0 0 ${SIZE} ${SIZE}`} width="100%" style={{ maxWidth: SIZE, display: 'block', margin: '0 auto' }}>
        <StyleDefs />
        {/* axes */}
        <line x1={PAD} y1={cy} x2={SIZE - PAD} y2={cy} data-stroke-role="axis" stroke={roles.axis.color} strokeWidth="1" />
        <line x1={cx} y1={PAD} x2={cx} y2={SIZE - PAD} data-stroke-role="axis" stroke={roles.axis.color} strokeWidth="1" />
        {ticks.map((v) => (
          <g key={v}>
            <line x1={X(v)} y1={cy - 3} x2={X(v)} y2={cy + 3} data-stroke-role="axis" stroke={roles.axis.color} />
            <line x1={X(-v)} y1={cy - 3} x2={X(-v)} y2={cy + 3} data-stroke-role="axis" stroke={roles.axis.color} />
            <line x1={cx - 3} y1={Y(v)} x2={cx + 3} y2={Y(v)} data-stroke-role="axis" stroke={roles.axis.color} />
            <line x1={cx - 3} y1={Y(-v)} x2={cx + 3} y2={Y(-v)} data-stroke-role="axis" stroke={roles.axis.color} />
          </g>
        ))}
        ...
        {/* cracking envelope */}
        {crack.length > 2 && (
          <path d={toPath(crack)} fill="none" data-stroke-role="stressBlockStroke" stroke={roles.stressBlockStroke.color} strokeWidth="1.5" strokeDasharray="5,3" />
        )}

        {/* strength envelope */}
        <path d={toPath(strength)} data-fill-role="concreteFill" data-stroke-role="concreteStroke" fill={roles.concreteFill.color} stroke={roles.concreteStroke.color} strokeWidth="1.8" />

        {/* NA-aligned anchors */}
        {[anchors.xSag, anchors.xHog, anchors.yPos, anchors.yNeg].map((a, i) => (
          <circle key={i} cx={X(a.phiMx)} cy={Y(a.phiMy)} r="3.5" data-fill-role="tensionSteel" data-stroke-role="dotStroke" fill={roles.tensionSteel.color} stroke={roles.dotStroke.color} strokeWidth="1" />
        ))}
        ...
        <g transform={`translate(${PAD}, ${SIZE - 6})`}>
          <line x1="0" y1="-4" x2="14" y2="-4" data-stroke-role="concreteStroke" stroke={roles.concreteStroke.color} strokeWidth="2" />
          <text x="18" y="-1" className="diagram-label legend-text">φMn strength</text>
          <line x1="100" y1="-4" x2="114" y2="-4" data-stroke-role="stressBlockStroke" stroke={roles.stressBlockStroke.color} strokeWidth="1.5" strokeDasharray="4,2" />
